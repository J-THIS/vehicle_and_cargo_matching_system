...
public class CheckBoxViewGroup<X extends TextView> extends ViewGroup {

    private HashSet<Integer> selectedSet;//当前被选中子控件的id集合
    ...
    //子控件内边距
    private int textModePaddingLeft = 20;
    private int textModePaddingTop = 10;
    private int textModePaddingRight = 20;
    private int textModePaddingBottom = 10;
    ...
    //获取当前被选中子控件的id
    public HashSet<Integer> getChosen_id() {
        return selectedSet;
    }

    //设置当前被选中子控件的id,与chooseItemStyle合用
    public void setChosen_id(int pos) {
        if (selectedSet.contains(pos)){//取消选中
            selectedSet.remove(pos);
            Log.i("selectedSet:REMOVE",pos+"");
        }else{//选中
            selectedSet.add(pos);
            Log.i("selectedSet:ADD",pos+"");
        }
    }
    ...
    public CheckBoxViewGroup(Context context, AttributeSet attrs) {
        super(context, attrs);
        mContext = context;
    }
    ...
    public void chooseItemStyle(int pos) {
        if (pos < getChildCount()) {
            if (selectedSet.contains(pos)){//再次点击取消选中
